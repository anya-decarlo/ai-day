#!/usr/bin/env python
# Training script for synthetic segmentation data without RL agent
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Train a segmentation model on synthetic data without RL agent")
    ...
    experiment_name = f"{args.optimizer}_{args.loss}_bs{args.batch_size}_lr{args.learning_rate:.6f}".replace(".", "p")
    results_dir = os.path.join(os.getcwd(), "results", "synthetic", f"{timestamp}_{experiment_name}")
    ...
    model = UNet(
        ...
        norm="instance",  # Use instance norm to match agent's starting point
    ).to(device)
    ...
    if args.loss == "Dice":
        ...
        # Use equal class weights to match agent's starting point
        ...
        loss_function = DiceCELoss(
            ...
            weight=class_weights  # Apply class weights to CE component
        )
    elif args.loss == "Focal":
        # Use equal class weights to match agent's starting point
        class_weights = torch.tensor([1.0, 1.0, 1.0], device=device)
        loss_function = FocalLoss(to_onehot_y=True, weight=class_weights, gamma=2.0)
    ...
    # No learning rate scheduler - use fixed learning rate
